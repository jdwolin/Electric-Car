...
void loop() {
    ...
    for(int x = 0; x >36; x++){
    returnedvoltage = RequestVoltage(x);  
    Serial.println (returnedvoltage, 4);  // print 4 digits of precision
    delay(500);
    }
}
// ***************************************************************************
  ...
  float RequestVoltage (int cellnumber){
    ...
    Wire.write("CV ");
    itoa(MyString, cellnumber, 10); // convert integer number to ascii characters, base 10
    Wire.write(MyString);
    ...
    MyString[i]=0;       // add a NULL character to the end of the string, may not be needed (already there) but doesn't hurt
